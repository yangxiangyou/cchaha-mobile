...
- 💾 **设备管理** — 多台电脑（设备）保存与切换；**打开 App / 点击设备都直接进入 cc-haha**；长按可**编辑**（地址/token/名称）、重命名、删除；地址与 token 分两行输入
- 🎨 **cc-haha 原版体验** — 与电脑端完全一致的 H5 界面，一套 UI 走到底（会话、消息、审批、附件全部原生网页交互）
- 🚪 **进页即见会话** — 自动展开 H5 会话抽屉，一进 App 就看到会话列表，不用手动点菜单
...
- 🚀 **启动加速** — 8 秒看门狗错误页自动屏蔽（兼容上游格式变化），加载慢也不报错闪屏
...
[cc-haha](https://github.com/NanmiCoder/cc-haha) 桌面应用内置本地 H5 服务（`设置 → H5 Access`）。手机用 WebView 直接打开这个 H5 地址遥控——会话列表、消息、权限审批、附件上传下载都在同一套界面上完成，与电脑端体验一致。数据不经过任何云端存储。
